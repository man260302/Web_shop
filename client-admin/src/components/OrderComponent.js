    ...
    const orders = this.state.orders.map((item) => {
      return (
        <tr key={item._id} className="datatable" onClick={() => this.trItemClick(item)}>
          ...
          <td>
            {item.status === 'PENDING' ?
              <div><span className="link" onClick={() => this.lnkApproveClick(item._id)}>CHẤP NHẬN</span> || <span className="link" onClick={() => this.lnkCancelClick(item._id)}>HỦY BỎ</span></div>
              : <div />}
          </td>
        </tr>
      );
    ...
    return (
      <div>
        ...
        <div className="align-left">
          <h2 className="text-left">DANH SÁCH ĐẶT HÀNG</h2>
        ...
        {this.state.order ?
          <div className="align-right">
            <h2 className="text-right">CHI TIẾT ĐẶT HÀNG</h2>
            <table className="datatable" border="1">
              <tbody>
                <tr className="datatable">
                  <th>No.</th>
